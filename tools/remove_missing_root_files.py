...
files = get_file(api, api.last_root, ROOT_DOC_SCHEMA)
file_uuids = [document.uuid for document in api.documents.values()] + [
...
with SlashR(False) as sr:
    ...
    for i, file in enumerate(files.files):
        sr.print(
            f"{Fore.CYAN}Checking {i + 1} of {files.items} "
    ...
    sr.print(
        f"{Fore.CYAN}Checked {files.items} items on your cloud. "
        f"{Fore.GREEN}valid: {len(valid_files)} "
        f"{Fore.RED}invalid: {len(invalid_files)}{Fore.RESET}"
    )

for invalid_file in invalid_files:
    sub_files = get_file(api, invalid_file.hash, invalid_file.rm_filename)
    missing_extensions = set()
    available_extensions = set()
    files_ok = 0
    metadata = None
    for sub_file in sub_files.files:
        extension = sub_file.uuid.rsplit(".", 1)[-1]
        if check_file_exists(api, sub_file.hash, sub_file.rm_filename, use_cache=False):
            files_ok += 1
            available_extensions.add(extension)
            if not sub_file.uuid.endswith(".metadata"):
                continue
            try:
                raw_metadata = get_file_contents(
                    api, sub_file.hash, sub_file.rm_filename
                )
            except:
                continue
    ...
    print(
        ...
        f"{Fore.MAGENTA}Files: {files_ok} / {sub_files.items} "
...
root = api.get_root()
update_root(
    api, {"broadcast": True, "generation": root["generation"], "hash": root_file.hash}
)
