...
class ContextBar(pe.ChildContext, ABC):
    ...
    def buttons(self) -> List[pe.RectButton]:
        ...
        for i, button in enumerate(self.BUTTONS):
            ...
            rect.inflate_ip(
                self.ratios.main_menu_x_padding, -self.ratios.main_menu_x_padding
            )
                ...
                (
                    pe.RectButton(
                        ...
                        (
                            ...
                            if button["action"]
                            else Defaults.TRANSPARENT_COLOR
                        ),
                        action_set={
                            "l_click": {
                            ...
                            "r_click": (
                                ...
                                if (context_menu := button.get("context_menu"))
                                else None
                            ),
                            "hover_draw": None,
                            "hover": None,
    ...
    def handle_action(self, action, data):
        self.quick_refresh()
        pe.button.Button.action_call({"action": action, "args": data})
    ...
    def handle_new_context_menu(self, context_menu_getter, index):
        if self.CONTEXT_MENU_OPEN_DIRECTION == "down":
            ...
            ideal_position = (
                ideal_position[0],
                ideal_position[1] + self.CONTEXT_MENU_OPEN_PADDING,
            )
        elif self.CONTEXT_MENU_OPEN_DIRECTION == "right":
            ideal_position = self.buttons[index].area.topright
            ideal_position = (
                ideal_position[0] + self.CONTEXT_MENU_OPEN_PADDING,
                ideal_position[1],
            )
        else:
            raise ValueError(
                f"Invalid CONTEXT_MENU_OPEN_DIRECTION '{self.CONTEXT_MENU_OPEN_DIRECTION}'"
            )
    ...
    def handle_scales(self):
        ...
        for (
            ...
            button_meta,
            button_text,
            button_text_inverted,
        ) in self.button_data_zipped:
        ...
        for (
            ...
            if context_icon_type := button_meta.get("context_icon"):
                if context_icon_type == "context_menu":
                    context_icon = (
                        self.icons["context_menu"]
                        if not is_inverted
                        else self.icons["context_menu_inverted"]
                    )
                    ...
                    context_icon = (
                        ...
                        if not is_inverted
                        else self.icons["chevron_right_inverted"]
                    )
                    ...
                    context_icon = (
                        ...
                        if not is_inverted
                        else self.icons["chevron_down_inverted"]
                    )
                    context_icon.display(button_meta["chevron_down_icon_rect"].topleft)
                elif context_icon_type == "small_chevron_right":
                    context_icon = (
                        self.icons["small_chevron_right"]
                        if not is_inverted
                        else self.icons["small_chevron_right_inverted"]
                    )
                    context_icon.display(
                        button_meta["small_chevron_right_icon_rect"].topleft
                    )
                elif context_icon_type == "small_chevron_down":
                    context_icon = (
                        self.icons["small_chevron_down"]
                        if not is_inverted
                        else self.icons["small_chevron_down_inverted"]
                    )
                    context_icon.display(
                        button_meta["small_chevron_down_icon_rect"].topleft
                    )
...
class FixedSizeContextBar(ContextBar, ABC):
    def handle_scales(self):
        ...
        for button, button_meta, text, text_inverted in self.button_data_zipped:
            text_scaled = new_lined_dynamic_text(
                text.text, text.font, button.area.width - text.rect.left
            )
