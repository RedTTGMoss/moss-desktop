...
def translation_pair(key: str) -> tuple[str, str]:
    ...
    return (f"popups.{key}.title", f"popups.{key}.description")
...
class Popup(pe.ChildContext, ButtonReadyMixin, TitledMixin):
    ...
    def __init__(self, parent: "GUI", title: str, description: str, **kwargs):
        ...
        self.description = pe.Text(
            t(description, None, **kwargs),
            Defaults.DEBUG_FONT,
            self.ratios.popup_description_size,
            colors=self.TITLE_COLORS,
        )
        self.description.rect.left = self.title.rect.left
        self.description.rect.top = (
            self.title.rect.bottom + self.ratios.popup_description_padding
        )
        self.closed = False
    ...
    def loop(self):
        ...
        pe.button.rect(
            ...
            self.COLOR,
            self.COLOR,
            name=f"popup_{id(self)}_background",
        )
        self.title.display()
...
class ConfirmPopup(Popup):
    ...
    def loop(self):
        ...
        render_button_using_text(
            ...
            active_color=Defaults.BUTTON_ACTIVE_COLOR_INVERTED,
            action=self.ok,
            name=f"popup_{id(self)}_confirm",
            text_infront=True,
        )
