...
class LIB_rM_Lines_ChunkTask:
    def __init__(
        self,
        renderer: "Renderer",
        agent: "LIB_rM_Lines_ChunkingAgent",
        chunk_rect: pe.Rect,
    ):
        self.renderer = renderer
        self.agent = agent
    ...
    def load(self):
        ...
        self.image = pe.Image(
            pe.pygame.image.frombytes(raw, self.chunk_rect.size, "RGBA")
        )
        self.loaded = True
...
class LIB_rM_Lines_ChunkingAgent:
    ...
    def __init__(
        ...
        renderer: "Renderer",
        base_rect: pe.Rect,
        expanded_notebook: "LIB_rM_Lines_ExpandedNotebook",
    ):
        self.renderer = renderer
    ...
    def get_chunks(self, rect: pe.Rect, size_multiplier: float = 1):
        ...
        chunk_count = math.ceil(rect.width / chunk_size) * math.ceil(
            rect.height / chunk_size
        )
        if chunk_count > 20:
        ...
        while y_offset < rect.height:
            while x_offset < rect.width:
                chunk_rect = pe.Rect(x_offset, y_offset, chunk_size, chunk_size)
        ...
        for chunk_rect in chunk_rects:
            ...
            if hash_of_chunk in self.cache:
                ...
                continue
            chunk = LIB_rM_Lines_ChunkTask(self.renderer, self, chunk_rect)
...
class LIB_rM_Lines_Preview:
    def __init__(
        self, renderer: "Renderer", frame_x: int, frame_y: int, lock: threading.Lock
    ):
        self._sprite: pe.Sprite = None
        threading.Thread(
            target=self.load, args=(renderer, frame_x, frame_y, lock), daemon=True
        ).start()
...
class LIB_rM_Lines_ExpandedNotebook(ExpandedNotebook):
    ...
    def get_frame_from_initial(
        ...
        if not self.previews.get(frame_hash := (frame_x, frame_y)):
            self.previews[frame_hash] = LIB_rM_Lines_Preview(
                self.renderer, frame_x, frame_y, self.lock
            )
...
class Notebook_LIB_rM_Lines_Renderer(AbstractRenderer):
    ...
    def _load(self, page_uuid: str):
        ...
        if self.tree:
            ...
            if not self.renderer.uuid:
                ...
                return
            self.renderer.template = self.document.content.c_pages.get_page_from_uuid(
                page_uuid
            ).template.value
    ...
    def check_page_loaded(self, page_uuid: str) -> bool:
        if (
            page_uuid not in self.unloadable_pages
            and (self.tree and self.tree.page_uuid != page_uuid)
    ...
    def render(self, page_uuid: str):
        ...
        for (frame_x, frame_y), frame_task in frames.items():
            ...
            preview_frame = (
                preview_sprite.get_finished_surface() if preview_sprite else None
            )
            ...
            for chunk in chunks:
                ...
                if chunk.loaded:
                    ...
                    pe.display.blit(
                        preview_frame, aligned_chunk_rect.topleft, chunk.chunk_rect
                    )
                if self.gui.config.debug_viewer:
                    pe.draw.rect(
                        pe.colors.red, aligned_chunk_rect, self.gui.ratios.line
                    )
