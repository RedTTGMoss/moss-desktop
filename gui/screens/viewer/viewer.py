...
class DocumentRenderer(pe.ChildContext):
    ...
    def __init__(self, parent: "GUI", document: "Document", ui: "DocumentViewerUI"):
        ...
        self.current_page_index = (
            self.document.content.c_pages.get_index_from_uuid(self.last_opened_uuid)
            or 0
        )
        self.renderer = None
        ...
        if self.config.notebook_render_mode == "rm_lines_svg_inker_OLD":
            ...
            self.close()
            print(
                f"{Fore.RED}Notebook render mode `{self.config.notebook_render_mode}` unavailable{Fore.RESET}"
            )
    ...
    def calculate_base_zoom(self):
        ...
        self.base_zoom = (
            self.maintain_aspect_size[0] / self.notebook_renderer.frame_width
        )
    ...
    def handle_navigation(self, event: pe.pygame.event.Event):
        if self.ctrl_hold and event.type == pe.pygame.MOUSEWHEEL:
                ...
                self.draggable.pos = self.pos = tuple(
                    int(
                        ...
                        - (
                            ...
                            * offset
                            * zoom_delta  # Finally multiply by the offset and the zoom delta
                        )
                    )  # Ensure the final value is an integer
                    for pos, mp, center, reference_size, offset in zip(
                        self.pos,
                        (
                            pe.mouse.pos()
                            if self.ui.mode == "free"
                            else tuple(v / 2 for v in self.size)
                        ),
                        self.center,
            ...
            self.draggable.pos = self.pos = tuple(
                v + scroll * m for v, m in zip(self.pos, multiplier)
            )

        if not self.hold_timer:
            if any(
                [pe.event.key_DOWN(key) for key in Defaults.NAVIGATION_KEYS["next"]]
            ):
                self.hold_next = True
                self.hold_timer = time.time() + self.PAGE_NAVIGATION_DELAY
                self.do_next()
            if any(
                [pe.event.key_DOWN(key) for key in Defaults.NAVIGATION_KEYS["previous"]]
            ):
            ...
            if any(
                [
                    ...
                    + Defaults.NAVIGATION_KEYS["previous"]
                ]
            ):
                self.hold_next = False
                self.hold_previous = False
    ...
    def align_top(self):
        ...
        self.draggable.pos = self.pos = (
            ...
            + (
                RM_SCREEN_CENTER[1]
                * self.zoom
                * self.ratios.rm_scaled(RM_SCREEN_SIZE[0])
            )
            + self.ratios.document_viewer_top_height
            + self.ratios.document_viewer_top_margin * 2,
        )
    ...
    def pre_loop(self):
        ...
        if self.loading:
            ...
            if section > 0.5:
                pe.draw.circle(
                    pe.colors.white,
                    (self.first_dot, self.loading_rect.centery),
                    self.ratios.document_viewer_loading_circle_radius,
                )
            if section > 2:
                pe.draw.circle(
                    pe.colors.white,
                    (self.second_dot, self.loading_rect.centery),
                    self.ratios.document_viewer_loading_circle_radius,
                )
            if section > 3:
                pe.draw.circle(
                    pe.colors.white,
                    (self.third_dot, self.loading_rect.centery),
                    self.ratios.document_viewer_loading_circle_radius,
                )
            if section > 3.5:
                self.loading_timer = time.time()
    ...
    @property
    def debug_text(self):
        if self.notebook_renderer and getattr(
            self.notebook_renderer, "expanded_notebook", None
        ):
            rm_position = tuple(
                (
                    (
...
class DocumentViewerUI(pe.ChildContext):
    LAYER = pe.BEFORE_POST_LAYER
    R_BUTTONS = (
        {"icon": "x_small", "hint": "viewer.close", "action": "close"},
        "DocumentModeToggler",
    )
    L_BUTTONS = ()
    ...
    def loop(self):
        ...
        pe.draw.rect(
            Defaults.SELECTED,
            self.rect,
            self.ratios.document_viewer_top_outline,
            edge_rounding=self.ratios.document_viewer_top_rounding
            - self.ratios.document_viewer_top_outline,
        )
        ...
        for right, items in zip(
            (True, False), (enumerate(self.r_buttons), enumerate(self.l_buttons))
        ):
            for i, button in items:
                ...
                pe.button.action(
                    rect,
                    name=f"document_viewer_x<{id(self.viewer)}><r{i}>",
                    action_set={
                        "l_click": {
                            "action": (
                                getattr(self, action)
    ...
    def get_hint_text(self, text):
        return pe.Text(
            ...
            self.ratios.document_viewer_hint_size,
    ...
    def align_rects(self):
        self.rect = pe.Rect(
            0,
            0,
            self.width - self.ratios.document_viewer_top_margin * 2,
            self.ratios.document_viewer_top_height,
        )
        self.rect.move_ip(
            self.ratios.document_viewer_top_margin,
            self.ratios.document_viewer_top_margin,
        )
...
class DocumentViewer(pe.ChildContext):
    ...
    def __init__(self, parent: "GUI", document_uuid: str):
        super().__init__(parent)
        top_rect = pe.Rect(
            0, 0, parent.width, parent.ratios.document_viewer_top_draggable_height
        )
        ...
        self.top_puller = DraggablePuller(
            parent,
            top_rect,
            detect_y=-top_rect.height,
            callback_y=self.close,
        ...
        if (
            ...
            and not self.config.guides.switch_to_librm_lines
        ):
    ...
    def init_renderer(self):
        try:
            self.document_renderer = DocumentRenderer(
                self.parent_context, self.document, self.ui
            )
    ...
    def resize_check_hook(self, event):
        if isinstance(event, ResizeEvent):
            ...
            self.top_puller.draggable.area = (
                event.new_size[0],
                self.top_puller.draggable.area[1],
            )
            self.ui.align_rects()
    ...
    def close(self):
        ...
        if self.config.save_after_close:
            ...
            self.document.content.c_pages.last_opened.value = (
                self.document_renderer.last_opened_uuid
            )
            self.document.metadata.last_opened_page = (
                self.document_renderer.current_page_index
            )
            self.document.metadata.last_opened = models.now_time()
            threading.Thread(
                target=self.api.upload, args=(self.document,), kwargs={"unload": True}
            ).start()
        else:
    ...
    def draw_close_indicator(self):
        ...
        bottom = min(
            self.height // 4, pe.mouse.pos()[1]
        )  # Limit the bottom to 1/4 of the screen

        # Calculate the center of the icon
        icon_rect.centerx = self.top_puller.rect.centerx
        icon_rect.centery = top + (bottom - top) * 0.5
        ...
        outline_rect = pe.Rect(
            0, 0, icon_rect.width, icon_rect.bottom - self.top_puller.rect.centery
        )
        ...
        pe.draw.line(
            Defaults.LINE_GRAY,
            (
                x_pos := self.top_puller.rect.centerx - self.ratios.pixel(2),
                self.top_puller.rect.centery,
            ),
            (x_pos, icon_rect.centery),
            self.ratios.pixel(3),
        )
