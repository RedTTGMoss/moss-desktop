...
class CustomInputBox(pe.InputBox):
    ...
    def draw_cursor(self, active_blink: bool):
        ...
        pe.draw.line(
            Defaults.LINE_GRAY,
            (self.cursor_x, self.text.rect.top),
            (self.cursor_x, self.text.rect.bottom),
            self.gui.ratios.line,
        )
...
class NameFieldScreen(pe.ChildContext, ButtonReadyMixin, TitledMixin):
    ...
    def __init__(
        ...
        self.field = CustomInputBox(
            ...
            text,
            gui.ratios.field_screen_input_size,
    ...
    def loop(self):
        ...
        render_button_using_text(
            ...
            action=self.close,
            name="name_field_screen.cancel",
        )

        render_button_using_text(
            self.parent_context,
            self.texts["ok_button"],
            outline=self.ratios.outline,
            action=self.ok,
            name="name_field_screen.ok_button",
