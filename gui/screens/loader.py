...
class Loader(pe.ChildContext, LogoMixin):
    TO_LOAD = {
        ...
        "folder_empty": InvertedIcon(
            os.path.join(Defaults.ICON_DIR, "folder_empty.svg")
        ),
        ...
        "chevron_down": InvertedIcon(
            os.path.join(Defaults.ICON_DIR, "chevron_down.svg")
        ),
        "chevron_right": InvertedIcon(
            os.path.join(Defaults.ICON_DIR, "chevron_right.svg")
        ),
        "chevron_left": InvertedIcon(
            os.path.join(Defaults.ICON_DIR, "chevron_left.svg")
        ),
        "small_chevron_down": InvertedIcon(
            os.path.join(Defaults.ICON_DIR, "small_chevron_down.svg")
        ),
        "small_chevron_right": InvertedIcon(
            os.path.join(Defaults.ICON_DIR, "small_chevron_right.svg")
        ),
        "cloud": InvertedIcon(os.path.join(Defaults.ICON_DIR, "cloud.svg")),
        "cloud_download": InvertedIcon(
            os.path.join(Defaults.ICON_DIR, "cloud_download.svg")
        ),
        "cloud_synced": InvertedIcon(
            os.path.join(Defaults.ICON_DIR, "cloud_synced.svg")
        ),
        ...
        "notebook_large": InvertedIcon(
            os.path.join(Defaults.ICON_DIR, "notebook_large.svg")
        ),
        "notebook": InvertedIcon(ReusedIcon("notebook_large", 0.333)),
        "notebook_add": InvertedIcon(
            os.path.join(Defaults.ICON_DIR, "notebook_add.svg")
        ),
        ...
        "picture": InvertedIcon(os.path.join(Defaults.ICON_DIR, "picture.svg")),
        ...
        "context_menu": InvertedIcon(
            os.path.join(Defaults.ICON_DIR, "context_menu.svg")
        ),
        ...
        "trashcan_delete": InvertedIcon(
            os.path.join(Defaults.ICON_DIR, "trashcan_delete.svg")
        ),
        ...
        **{
            f"templates/{template.rsplit('.', 1)[0]}": TreatAsData(
                os.path.join(Defaults.DATA_DIR, "templates", template)
            )
        ...
        "xml_missing_menu_template": os.path.join(
            Defaults.XML_DIR, "missing_menu_template.xml"
        ),
        **{
            f"xml_settings/{settings_sub_menu.rsplit('.', 1)[0]}": TreatAsData(
                os.path.join(
                    Defaults.XML_DIR, "settings", "sub_menus", settings_sub_menu
                )
            )
            for settings_sub_menu in os.listdir(
                os.path.join(Defaults.XML_DIR, "settings", "sub_menus")
            )
        },
    }
    ...
    def __init__(self, parent: "GUI"):
        ...
        try:
            ...
            self.motivational_message = self.motivational_message.replace(
                "%APP_NAME%", APP_NAME
            )
    ...
    def _load(self):
        ...
        while not (
            self.extension_manager.gathered_extensions
            and self.extension_manager.extensions_loaded
    ...
    def load_image(self, key, file, multiplier: float = 1):
        ...
        self.icons[key].resize(
            tuple(self.ratios.pixel(v * multiplier) for v in self.icons[key].size)
        )
    ...
    def progress(self):
        if (
            self.files_to_load is None
            and self.config.wait_for_everything_to_load
            and not self.current_progress
        ):
            # Before we know the file count, just return 0 progress
            return 0
        try:
            base_division_a = self.extensions_loaded + self.items_loaded
            base_division_b = self.extension_count + self.to_load
            if not self.config.wait_for_everything_to_load:
                self.current_progress = base_division_a / base_division_b
            else:
                self.current_progress = (base_division_a + self.files_loaded) / (
                    base_division_b + (self.files_to_load or self.files_loaded)
                )
        except ZeroDivisionError:
            self.current_progress = 0
        self.last_progress = self.last_progress + (
            self.current_progress - self.last_progress
        ) / (self.FPS * 0.1)
        if (
            not self.config.wait_for_everything_to_load
            and self.current_progress == 1
            and self.last_progress < 0.9
        ):
    ...
    def loop(self):
        ...
        if progress:
            pe.draw.rect(
                Defaults.LINE_GRAY,
                self.line_rect,
                self.ratios.loader_loading_bar_thickness,
            ...
            if self.config.debug:
                ...
                pe.draw.rect(
                    ...
                    icons_rect,
                    self.ratios.loader_loading_bar_thickness * 2,
                    edge_rounding=self.ratios.loader_loading_bar_rounding,
                )

    def post_loop(self):
        if self.current_progress == 1 and self.last_progress >= 0.98:
            ...
            self.api.add_hook(
                "loader", self.loader_hook
            )  # Add api event hook for loader
            try:
