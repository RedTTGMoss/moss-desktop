...
from gui.events import (
    ResizeEvent,
    ImportScreenLightSyncInit,
    ImportScreenLightSyncComplete,
    ...
    def documents(self):
        return {
            **{
                ...
                for document in self.import_screen.documents_to_upload
            },
            **{
                dummy_document.uuid: dummy_document
                for dummy_document in self.import_screen.dummy_documents
            },
        }
...
class ImportScreen(pe.ChildContext, ButtonReadyMixin, TitledMixin):
    ...
    def predefine_item(self, items: int = 1):
        ...
        for _ in range(items):
            ...
            self.dummy_documents[-1].metadata.last_modified += timedelta(
                weeks=10
            ).total_seconds()
    ...
    def loop(self):
        ...
        not_ready = (
            self.uploading_light
            or len(self.documents_to_upload) < self.expected_documents
        )
        ...
        render_button_using_text(
            ...
            name="import_screen.full_sync",
            disabled=(0, 0, 0, 50) if not_ready else False,
        )
        light_import_rect = render_button_using_text(
            self.parent_context,
            self.texts["light"],
            outline=self.ratios.outline,
            action=self.light_import,
            name="import_screen.light_sync",
            disabled=(0, 0, 0, 50) if not_ready else False,
        )
    ...
    def full_import(self):
        self.api.spread_event(
            ImportScreenFullSyncConfirm(self.document_uuids, id(self))
        )
        threading.Thread(
    ...
    def light_import(self):
        ...
        threading.Thread(target=self._light_import, daemon=True).start()
    ...
    def _light_import(self):
        ...
        self.uploading_light = False
        self.api.spread_event(
            ImportScreenLightSyncComplete(self.document_uuids, id(self))
        )
    ...
    def convert_light(self):
        ...
        for document in self.documents_to_upload:
            ...
            if document.content.file_type == "pdf":
                light_documents.append(
                    document.replace_pdf(
                        surfaces_to_pdf([preview.get_finished_surface()])
                        if preview
                        else self.data["light_pdf"]
                    )
                )
