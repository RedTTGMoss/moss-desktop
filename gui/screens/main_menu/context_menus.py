...
class ImportContextMenu(ContextMenu):
    ...
    def _notebook_import(self, title: str):
        notebook_prompt(
            lambda file_paths: import_notebook_pages_to_cloud(
                self.main_menu, file_paths, title
            )
        )
...
class ExportContextMenu(ContextMenu):
    BUTTONS = (
        {"text": "menu.export.rmdoc", "icon": "notebook", "action": "export_rmdoc"},
        {"text": "menu.export.png", "icon": "picture", "action": "export_png"},
    )
    api: "API"
    ...
    def _export_rmdoc(self, file_path: str):
        ...
        with zipfile.ZipFile(file_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
            for file in document.files:
                zf.write(
                    os.path.join(self.api.sync_file_path, file.hash), arcname=file.uuid
                )
...
class DebugContextMenu(ContextMenu):
    ...
    BUTTONS = (
        {"text": "menu.debug.global", "icon": "cog", "action": None},
        {"text": "menu.debug.test_doc", "icon": "notebook", "action": "test_doc_view"},
        {
        ...
        {"text": "menu.debug.hot_reload", "icon": "rotate", "action": "hot_reload"},
        {
            "text": "menu.debug.clear_preview",
            "icon": "trashcan_delete",
            "action": "clear_preview",
        },
        {"text": "menu.debug.copy_uuid", "icon": "copy", "action": "copy_uuid"},
        {
    ...
    def test_doc_view(self):
        if not PreviewHandler.CACHED_PREVIEW.get(self.DEBUG_PREVIEW):
            ...
            PreviewHandler.CACHED_PREVIEW[self.DEBUG_PREVIEW] = (
                self.DEBUG_PREVIEW_PAGE_INDEX,
                pe.Sprite(surface),
            )
        ...
        self.apply(
            folder_with_tag := DocumentCollection.create(
                self.api, "Folder /w tag", self.DEBUG_FOLDER
            )
        )
        self.apply(
            folder_with_star := DocumentCollection.create(
                self.api, "Folder /w star", self.DEBUG_FOLDER
            )
        )
        self.apply(
            Document.new_notebook(self.api, "Normal notebook", self.DEBUG_FOLDER)
        )
        self.apply(
            notebook_with_tag := Document.new_notebook(
                self.api, "Notebook /w tag", self.DEBUG_FOLDER
            )
        )
        self.apply(
            notebook_with_star := Document.new_notebook(
                self.api, "Notebook /w star", self.DEBUG_FOLDER
            )
        )
        self.apply(
            problematic_notebook := Document.new_notebook(
                self.api, "Problematic notebook", self.DEBUG_FOLDER
            )
        )
        self.apply(
            syncing_notebook := Document.new_notebook(
                self.api, "Syncing notebook", self.DEBUG_FOLDER
            )
        )

        test_tag = Tag({"name": "Test Tag", "timestamp": "1:0"})
    ...
    def apply(self, item):
        if isinstance(item, DocumentCollection):
            ...
            PreviewHandler.CACHED_PREVIEW[item.uuid] = PreviewHandler.CACHED_PREVIEW[
                self.DEBUG_PREVIEW
            ]
    ...
    def _import_from_directory(self, file_paths: List[str]):
        ...
        for file_path in file_paths:
            if file_path.endswith(".rm"):
                ...
                with open(file_path, "r") as f:
                    ...
                    content = Content(
                        raw_content, metadata, make_hash(raw_content), True
                    )
            elif file_path.endswith(".metadata.json") or file_path.endswith(
                ".metadata"
            ):
                with open(file_path, "r") as f:
        ...
        document = Document.new_notebook(
            ...
            notebook_data=[FileHandle(file) for file in rm_files],
    ...
    def export_docs_rm(self):
        for document in self.main_menu.documents.values():
            ...
            export_path = os.path.join(
                Defaults.SYNC_EXPORTS_FILE_PATH,
                self.main_menu.navigation_parent,
...
class CustomExtensionsMenu(ContextMenu):
    ...
    DEFAULT_BUTTONS = (
        {"text": "sidebar.no_extensions", "icon": "puzzle", "action": None},
    )
    NO_EXTENSION_MANAGER = (
        {"text": "Extension manager failed to load", "icon": "puzzle", "action": None},
    )
    ...
    def get_actions(self) -> List[Tuple[Optional[str], Optional[str], str]]:
        return [
            (button.get("action"), button.get("context_menu"), button["_extension"])
    ...
    def __getattr__(self, item):
        if item.startswith(ACTION_APPEND):
            for action, context_menu, extension in self.get_actions():
                if item == action:
                    return self.extension_manager.action(
                        action[len(ACTION_APPEND) :], extension
                    )
                if item == context_menu:
                    function = self.extension_manager.action(
                        context_menu[len(ACTION_APPEND) :], extension
                    )
...
class SideBar(FixedSizeContextBar, ContextMenu):
    ...
    BUTTONS = (
        ...
        {"text": "sidebar.made_by", "icon": "heart", "action": None},
        {
    ...
    def __init__(self, context, *args, **kwargs):
        if context.config.debug:
            ...
            self.BUTTONS = (*self.BUTTONS[:4], *self.BUTTONS[5:])
            self.TOP_BUTTONS = 4
    ...
    def finalize_button_rect(self, buttons, width, height):
        ...
        for button in reversed(buttons[self.TOP_BUTTONS :]):
            ...
            y -= button.area.height
        self.rect = pe.Rect(
            self.left, self.top, self.ratios.main_menu_side_bar_width, self.height
        )
    ...
    def button_margin(self):
        return (
            self.main_menu.bar.buttons[0].area.left + self.main_menu.bar.button_padding
        )
...
class LanguageMenu(ContextMenu):
    ...
    def __init__(self, parent: "MainMenu", ideal_position: Tuple[int, int]):
        self.BUTTONS = [
            {"text": lang, "action": "set_language", "icon": "language", "data": lang}
