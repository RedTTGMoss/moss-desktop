...
class Installer(pe.ChildContext, LogoMixin):
    ...
    def __init__(self, parent: "GUI"):
        ...
        self.cancel_text, self.install_text, self.reinstall_text, self.launch_text = (
            tuple(
                pe.Text(
                    text,
                    Defaults.INSTALLER_FONT,
                    self.ratios.installer_buttons_size,
                    colors=Defaults.TEXT_COLOR_T,
                )
                for text in ("Cancel", "Install", "Reinstall", "Launch")
            )
        )
    ...
    def calculate_texts(self):
        ...
        buttons_rect = pe.Rect(
            0,
            0,
            self.ratios.installer_buttons_width,
            self.ratios.installer_buttons_height,
        )
        buttons_rect.midtop = self.line_rect.midtop
        self.cancel_button_rect = buttons_rect.scale_by(0.5, 1)
        ...
        if self.installing:
            ...
            pe.button.rect(
                ...
                (
                    Defaults.TRANSPARENT_COLOR
                    if can_install
                    else Defaults.BUTTON_DISABLED_COLOR
                ),
                Defaults.BUTTON_ACTIVE_COLOR,
                action=self.install,
                text=(
                    self.install_text if not Defaults.INSTALLED else self.reinstall_text
                ),
    ...
    def install_thread(self):
        ...
        for root, dirs, files in (
            os.walk(self.from_directory) if not self.config.debug else ()
        ):
    ...
    def make_link(self, path):
        ...
        with winshell.shortcut(path) as link:
            ...
            link.icon_location = (
                os.path.join(INSTALL_DIR, "assets", "icons", "moss.ico"),
                0,
            )
    ...
    def copy_config(self):
        ...
        if os.path.exists(Defaults.TOKEN_FILE_PATH) and not os.path.exists(
            to := os.path.join(USER_DATA_DIR, "token")
        ):
