                ...
                self.versions = {
                    ...
                    for package, version in map(
                        lambda line: (
                            str.split(line, "==")
                            if "==" in line
                            else str.split(line, "~=") if "~=" in line else (None, None)
                        ),
    ...
    def check(self):
        ...
        if os.path.exists(".git"):
            ...
            try:
                ...
                status = (
                    subprocess.check_output(["git", "status", "-sb"])
                    .strip()
                    .decode("utf-8")
                )
                if "[behind" in status:
                    ...
                    self.warnings.put(
                        WarningPopup(
                            self.parent_context, *translation_pair("warnings.git_ahead")
                        )
                    )
        ...
        for extension_name in os.listdir(Defaults.EXTENSIONS_DIR):
            if extension_name.endswith(".zip"):
                extension_directory = os.path.join(
                    Defaults.EXTENSIONS_DIR, extension_name[:-4]
                )
                self.warnings.put(
                    InstallPopup(
                        self.parent_context,
