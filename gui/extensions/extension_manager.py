...
def output_to_dict(func):
    def wrapper(self: "ExtensionManager", output, *args, **kwargs):
        try:
            ...
            self.error(
                f"Expected a serializable object from {self.current_extension}. Instead got: {bytes(output)}"
            )
            raise
...
class ExtensionManager:
        ...
        for extension_name, extension in self.extensions.items():
            ...
            try:
                if self.lock.locked():
                    ...
                    if self.lock.locked():
                        self.error(
                            f"Extension {extension_name} failed to unregister due to extension locking."
                        )
                        continue
                with self.lock:
    ...
    def load_wasm(
        ...
        if not os.path.exists(default_config_path):
            ...
            try:
                ...
                self.warn(
                    f"Extension {extension_path} default config could not be parsed."
                )
                default_config = {}
        if not os.path.exists(config_path):
            ...
            try:
                ...
                self.warn(
                    f"Extension {extension_path} config could not be parsed. Resetting the config!"
                )
                self.configs[extension_name] = default_config
    ...
    def load_wasm_source(self, source: bytes, extension_name: str):
        allowed_paths = {
            ...
            (
                extension_dir := os.path.join(Defaults.EXTENSIONS_DIR, extension_name)
            ): "extension",
        ...
        extension = Plugin(
            ...
            True,
            functions=[fn for fn in HOST_FN_REGISTRY if getattr(fn, "moss", False)],
        )
        ...
        if (
            settings_button_json_path := Path(extension_dir)
            / "assets"
            / "settings_button.json"
        ).exists():
            try:
                with open(settings_button_json_path, "r") as f:
                    button = json.load(f)
                    if all(
                        (key in button for key in ("text", "icon", "action", "data"))
                    ):
                        button["text"] = t(button["text"])
                        self.extension_menus[extension_name] = button
                    else:
                        self.warn(
                            f"Extension {extension_name} settings button contains insufficient keys."
                        )
            except JSONDecodeError:
                self.warn(
                    f"Extension {extension_name} settings button could not be parsed."
                )
    ...
    def gather_extensions(self):
        for extension_name in os.listdir(Defaults.EXTENSIONS_DIR):
            ...
            if extension_name.endswith(".wasm"):
                ...
                os.makedirs(
                    os.path.join(Defaults.EXTENSIONS_DIR, extension_name), exist_ok=True
                )
                os.rename(
                    os.path.join(Defaults.EXTENSIONS_DIR, f"{extension_name}.wasm"),
                    os.path.join(
                        Defaults.EXTENSIONS_DIR,
                        extension_name,
                        f"{extension_name}.wasm",
                    ),
                )
    ...
    def _load(self):
        ...
        for extension in self.extensions_to_load:
            ...
            wasm_file_location = os.path.join(
                Defaults.EXTENSIONS_DIR, extension, f"{extension}.wasm"
            )
            info_file_location = os.path.join(
                Defaults.EXTENSIONS_DIR, extension, f"{extension}.info"
            )
            if os.path.exists(info_file_location):
                ...
                if file_location := info.get("file"):
                    wasm_file_location = os.path.join(
                        Defaults.EXTENSIONS_DIR, extension, file_location
                    )
                    if not wasm_file_location.startswith(
                        os.path.join(Defaults.EXTENSIONS_DIR, extension)
                    ):
                        self.error(
                            f"Extension {extension} file location is invalid or dangerous"
                        )
                        self.extension_count -= 1
                        continue
            ...
            if self.gui.config["extensions"][extension]:
                try:
                    self.load_wasm(
                        wasm_file_location,
                        os.path.join(
                            Defaults.EXTENSIONS_DIR, extension, f"options.json"
                        ),
                ...
                self.extension_count -= 1
                self.error(
                    f"Extension {extension} is disabled, enable it in the config!"
                )
    ...
    def loop(self):
        ...
        for extension_name, extension in self.extensions.items():
            ...
            self.current_extension = extension_name
            try:
                extension.call("moss_extension_loop", self.state)
            except ExtismError:
    ...
    def action(self, action: str, extension_name: str = None):
        ...
        def _action(**kwargs):
            with self.lock:
                ...
                try:
                    arg: Optional[str] = (
                        kwargs.pop("_arg") if "_arg" in kwargs else None
                    )
                    self.extensions[extension_name].call(
                        action,
                        (
                            arg
                            if arg
                            else json.dumps(kwargs).encode("utf-8") if kwargs else b""
                        ),
                    )
                except ExtismError:
                    self.error(
                        f"Extension {extension_name} failed to handle action {action}"
                    )
                    print_exc()
    ...
    def callback(
        self, callback: str, extension_name: str = None, data: Union[str, bytes] = None
    ) -> Union[None, tuple[partial[None], Union[int, Any]], partial[None]]:
        if not data:
        ...
        final_call = partial(
            ...
            _arg=(
                data.to_bytes(8, byteorder="little", signed=False)
                if return_task_id
                else data
            ),
        )
    ...
    def raw_state(self):
        return {
            ...
            "current_screen": (
                self.gui.current_screen.__class__.__name__ if self.gui.screens else ""
            ),
    ...
    def save_configs(self):
        for config, config_path in map(
            lambda extension_name: (
                ...
                os.path.join(Defaults.OPTIONS_DIR, f"{extension_name}.json"),
            ),
            self.dirty_configs,
        ):
            if not config:
