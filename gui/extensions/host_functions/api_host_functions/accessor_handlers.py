...
class SyncExtensionFunctionHelper:
    ...
    def __dict__(self):
        return {"text": self.text, "icon": self.icon, "accessor": self.accessor}
...
def uuid_accessor_adder(accessor_type: str, dictionary: dict):
    ...
    """
    dictionary["accessor"] = {"type": accessor_type, "uuid": dictionary["uuid"]}
...
def parent_accessor_adder(
    accessor_type: str,
    parent: Optional[Union[Document, DocumentCollection]],
    dictionary: dict,
    object_id: int = None,
):
    """
...
def document_inferred(accessor: AccessorInstanceBox) -> Tuple[Document, partial]:
    ...
    """
    getters = {
        AccessorTypes.APIDocument: lambda _: (
            d.api.documents[_.uuid],
            partial(uuid_accessor_adder, _.type),
        ),
        AccessorTypes.StandaloneDocument: lambda _: (
            d.extension_manager.document_objects[_.uuid],
            partial(uuid_accessor_adder, _.type),
        ),
    }
    ...
    if (getter := getters.get(AccessorTypes(accessor.type))) is not None:
        ...
        raise NotImplementedError(
            "This accessor type is not supported for document inferring"
        )


def collection_inferred(
    accessor: AccessorInstanceBox,
) -> Tuple[DocumentCollection, partial]:
    """
    ...
    getters = {
        AccessorTypes.APICollection: lambda _: (
            d.api.document_collections[_.uuid],
            partial(uuid_accessor_adder, _.type),
        ),
        AccessorTypes.StandaloneCollection: lambda _: (
            d.extension_manager.collection_objects[_.uuid],
            partial(uuid_accessor_adder, _.type),
        ),
    }
    ...
    if (getter := getters.get(AccessorTypes(accessor.type))) is not None:
        ...
        raise NotImplementedError(
            "This accessor type is not supported for collection inferring"
        )
...
def metadata_inferred(accessor: AccessorInstanceBox) -> Tuple[Metadata, partial]:
    ...
    getters = {
        AccessorTypes.APIDocumentMetadata: lambda _: (
            (parent := d.api.documents[_.uuid]).metadata,
            partial(parent_accessor_adder, _.type, parent),
        ),
        AccessorTypes.APICollectionMetadata: lambda _: (
            (parent := d.api.document_collections[_.uuid]).metadata,
            partial(parent_accessor_adder, _.type, parent),
        ),
    ...
    if (getter := getters.get(AccessorTypes(accessor.type))) is not None:
        ...
        raise NotImplementedError(
            "This accessor type is not supported for metadata inferring"
        )
...
def content_inferred(accessor: AccessorInstanceBox):
    ...
    getters = {
        AccessorTypes.APIDocumentContent: lambda _: (
            (parent := d.api.documents[_.uuid]).content,
            partial(parent_accessor_adder, _.type, parent),
        ),
    ...
    if (getter := getters.get(AccessorTypes(accessor.type))) is not None:
        ...
        raise NotImplementedError(
            "This accessor type is not supported for content inferring"
        )
...
def file_sync_progress_inferred(accessor: AccessorInstanceBox):
    ...
    if (getter := getters.get(AccessorTypes(accessor.type))) is not None:
        ...
        raise NotImplementedError(
            "This accessor type is not supported for FileSyncProgres inferring"
        )
...
def document_sync_progress_inferred(accessor: AccessorInstanceBox):
    ...
    if (getter := getters.get(AccessorTypes(accessor.type))) is not None:
        ...
        raise NotImplementedError(
            "This accessor type is not supported for DocumentSyncProgres inferring"
        )
...
def event_inferred(accessor: AccessorInstanceBox):
    ...
    getters = {
        AccessorTypes.FileSyncProgress: lambda _: (
            d.extension_manager.file_sync_progress_objects[_.id],
            None,
        ),
        AccessorTypes.DocumentSyncProgress: lambda _: (
            d.extension_manager.document_sync_progress_objects[_.id],
            None,
        ),
    ...
    if (getter := getters.get(AccessorTypes(accessor.type))) is not None:
        ...
        raise NotImplementedError(
            "This accessor type is not supported for Event inferring"
        )
...
def sync_stage_inferred(accessor: AccessorInstanceBox):
    ...
    getters = {
        AccessorTypes.SyncStage: lambda _: (
            obj := SyncExtensionFunctionHelper(_),
            obj.add_accessor,
        ),
    }
