...
def moss_pe_draw_rect(draw: Annotated[TPygameExtraRect, Json]):
    color, rect, width, edge_rounding = (
        draw["color"],
        draw["rect"],
        draw["width"],
        draw["edge_rounding"] or {},
    )
    pe.draw.rect(
        color_to_tuple(color),
        rect_to_pe_rect(rect),
        width,
        ...
        edge_rounding_bottomright=edge_rounding.get("edge_rounding_bottomright", -1)
        or -1,
        edge_rounding_bottomleft=edge_rounding.get("edge_rounding_bottomleft", -1)
        or -1,
    )
...
def moss_pe_register_screen(screen: Annotated[TScreen, Json]):
    ...
    d.extension_manager.log(
        f"Registered screen {d.extension_manager.current_extension}.{screen.key}"
    )
