...
def import_files_to_cloud(gui: "GUI", files):
    files = tuple(
        file
        for file in files
        if file.endswith(".pdf") or file.endswith(".epub") or file.endswith(".rmdoc")
    )
...
def import_notebook_pages_to_cloud(gui: "GUI", files: List[str], title: str):
    ...
    document = Document.new_notebook(
        ...
        notebook_data=[FileHandle(file) for file in files],
    )
...
def surfaces_to_pdf(surfaces: List[pe.Surface]):
    ...
    images[0].save(
        pdf_bytes, "PDF", save_all=True, resolution=100.0, append_images=images[1:]
    )
