...
for /f "delims=" %%i in ('python3 -c "import os, extism_sys; print(os.path.dirname(extism_sys.__file__))"') do set EXTISM_DIR=%%i

for /f "delims=" %%i in ('python3 -c "import os, pygameextra; print(os.path.dirname(pygameextra.pygame.__file__))"') do set PYGAME_DIR=%%i
for /f "delims=" %%i in ('python3 -c "import os, pygameextra; print(os.path.dirname(pygameextra.__file__))"') do set PYGAMEEXTRA_DIR=%%i
